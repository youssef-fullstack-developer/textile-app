...
@section('content')
    @php
        $route_main = 'sizing_plan';
        $module_label = 'Sizing Plan';
        $permission_name = 'sizing_plan';
    @endphp
    <div class="card card-custom" style="box-shadow: none">
        <div class="card-header flex-wrap border-0 pt-0 pb-0">
            ...
            <div class="card-toolbar">
                ...
                <a href="{{ route('sizing_plan.create') }}" class="btn btn-primary font-weight-bolder mr-2">
            ...
            <table class="table table-bordered table-hover" id="table_invoice_settings">
                ...
                @if (!empty($list))
                    @foreach ($list as $key => $item)
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>{{ $item->planning_date }}</td>
                            <td>{{ $item->plan_number }}</td>
                            <td>{{ $item->sizing_name?->name }}</td>
                            <td>{{ $item->yarn?->name }}</td>
                            <td>{{ $item->sizing_for }}</td>
                            <td>{{ $item->given_meters }}</td>
{{--                            @php--}}
{{--                                $no_of_beams = 0;--}}
{{--                                foreach ($item->beam_details as $row){--}}
{{--                                    $no_of_beams += ($row->beam_meters ?? 0);--}}
{{--                                }--}}
{{--                                    @endphp--}}
                            <td>{{ count(($item->beam_details ?? array())) }}</td>
                            <td>{{ $item->recieved_meters }}</td>
                            <td class="d-flex">
                                @can($permission_name.'-update')
                                <a href="{{ route('sizing_plan.edit', $item->id) }}"
