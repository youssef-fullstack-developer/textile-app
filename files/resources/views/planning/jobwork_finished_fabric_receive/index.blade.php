...
@section('content')
    @php
        $route_main = 'jobwork_finished_fabric_receive';
        $module_label = 'JOBWORK FINISHED FABRIC RECEIVE';
        $permission_name = 'jobwork_finished_fabric_receive';
    @endphp
    @php($route = 'jobwork_finished_fabric_receive')
        ...
        <div class="card-body">
            <table class="table table-bordered table-hover" id="table_invoice_settings">
                ...
                <tr class="text-uppercase">
                    <th class="pl-7"><span class="text-dark-75">JOBWORK CONTACT NO (PO NO)</span></th>
                    ...
                    <th class="pl-7"><span class="text-dark-75">VENDOR LOT NUMBER</span></th>
                    ...
                    @foreach ($list as $key => $item)
                        <tr>
                            ...
                            @foreach($item->details ?? array() as $row)
                                @php($pieces += $row->piece_no)
                                @php($length += $row->piece_meter)
                                @php($quality .= $row->quality?->details .'<br>')
                                @php($vendor_lot .= $row->vendor_lot_number .'<br>')
                            @endforeach
