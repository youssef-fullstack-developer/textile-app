...
@section('content')
    @php
        $route_main = 'yarn_inward';
        $module_label = 'Yarn Inward';
        $permission_name = 'yarn_inward';
    @endphp
    <div class="card card-custom" style="box-shadow: none">
        <div class="card-header flex-wrap border-0 pt-0 pb-0">
            ...
            <div class="card-toolbar">
                ...
                <a href="{{ route('yarn_inward.create') }}" class="btn btn-primary font-weight-bolder mr-2">
            ...
            <table class="table table-bordered table-hover" id="table_invoice_settings">
                ...
                @if (!empty($list))
                    @foreach ($list as $key => $item)
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>{{ $item->yarn_inward?->id }}</td>
                            <td>{{ $item->yarn_inward?->date }}</td>
                            <td>{{ $item->yarn_inward?->gate_pass_no }}</td>
                            <td>{{ $item->yarn_inward?->gate_pass_date }}</td>
                            <td>{{ $item->yarn?->name }}</td>
                            <td>{{ $item->mill?->name }}</td>
                            <td>{{ $item->yarn_inward?->vendor_group?->group }}</td>
                            ...
                            <td>{{ $item->yarn_inward?->location?->name }}</td>
                            <td class="d-flex">
                                @can($permission_name.'-update')
                                <a href="{{ route('yarn_inward.edit', $item->yarn_inward?->id) }}"
