            ...
            <div class="card card-custom ">
                <div class="card-header">
                    <div class="card-title">
                        <h3 class="card-label">
                            {{ $item?->id > 0 ? 'Edit' : 'Add' }} YARN INWARD
                        </h3>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-4">
                            <label>Po No <b class="text-danger">*</b></label>
                            <select name="purchase_order_id" id="purchase_order_id" class="form-control"
                                    onchange="change_purchase_order()" required>
                                {{--                                <option value="">Select</option>--}}
                                {{--                                @foreach($purchase_orders as $row)--}}
                                {{--                                    <option--}}
                                {{--                                        value="{{$row->id}}" {{ ($item?->purchase_order_id == $row->id) ? 'selected' : '' }}--}}
                                {{--                                    >{{$row->po_number}}</option>--}}
                                {{--                                @endforeach--}}
                                <option
                                    value="{{$item ? $item?->purchase_order?->id : '' }}"
                                >{{$item ? $item?->purchase_order?->po_number .'-'. $item?->purchase_order?->_po_number: 'Select' }}</option>
                            </select>
                        </div>
                        <div class="col-4">
                            <label>Agent</label>
                    ...
                    <div class="form-group row">
                        <div class="col-4">
                            <label>SO No</label>
                            <select name="sale_order_id" class="form-control">
                                <option value="">Select</option>
                                @foreach($sale_orders as $row)
                                    <option
                                        value="{{$row->id}}" {{ ($item?->sale_order_id == $row->id) ? 'selected' : '' }}
                                    >{{$row->order_no}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-2">
                            ...
                            <input type="text" name="receipt_number" class="form-control"
                                   value="{{$item ? $item?->receipt_number : '' }}"/>
                        </div>
                        <div class="col-4">
                            <label>Remarks</label>
                    ...
                    <div class="form-group row">
                        <div class="col-4">
                            ...
                            <input type="text" name="lorry_no" class="form-control"
                                   value="{{$item ? $item?->lorry_no : '' }}"/>
                        </div>
                        <div class="col-4">
                            <label>Lorry Weight</label>
                            <input type="number" name="lorry_weight" class="form-control"
                                   value="{{$item ? $item?->lorry_weight : '' }}"/>
                        </div>
                        <div class="col-4">
                            <label>Lorry Empty Weight</label>
                            <input type="number" name="lorry_empty_weight" class="form-control"
                                   value="{{$item ? $item?->lorry_empty_weight : '' }}"/>
                        </div>
                    </div>
                    {{-- Line 6 --}}
                    <div class="form-group row">
                        <div class="col-4">
                            ...
                            <input type="checkbox" name="is_jobwork_order" class="h-20px w-50px" value="1"
                    ...
                    <div id="details_container">
                        ...
                        @while( (empty($item) &&  $i == 0) || (count($item?->details ?? []) > $i ) )
                            ...
                            <div class="form-group border p-3 border-dark border-2 tr">
                                ...
                                <div class="form-group row">
                                    <div class="col-6">
                                        <label>Total No Of Bags / Box<b class="text-danger">*</b></label>
                                        <input type="number" name="details[{{$i}}][total_no_of_bags]"
                                               class="form-control" required
                                               value="{{$detail ? $detail?->total_no_of_bags : '' }}"/>
                                    </div>
                                    <div class="col-4">
                                        <label>Kgs Per Bag / Box(Bag / Box Weight)<b class="text-danger">*</b></label>
                                    ...
                                    <div class="col-12">
                                        <table class="table table-bordered table-hover">
                                            ...
                                            <tr class="table-primary">
                                                ...
                                                <th>LOT NO.</th>
                                                <th>NO. OF BAGS / BOX<b class="text-danger">*</b></th>
                                                <th>KGS PER BAG / BOX<b class="text-danger">*</b></th>
                                                <th>TOTAL KGS<b class="text-danger">*</b></th>
                                                <th>CONES PER BAG / BOX<b class="text-danger">*</b></th>
                                                <th>
                                            ...
                                            @if(!empty($detail))
                                                @foreach($detail?->lots as $index => $row)
                                                    <tr>
                                                        <td>
                                                            <select
                                                                name="details[{{$i}}][lots][{{$index}}][cone_tip_color_id]"
                                                                class="form-control">
                                                                <option value="">Select</option>
...
@push('scripts')
    <script>
        const add_detail_row = () => {
            ...
            html += `        <div class="form-group row">`;
            html += `        <div class="col-6">`;
            html += `            <label>Total No Of Bags / Box<b class="text-danger">*</b></label>`;
            html += `            <input type="number" name="details[${i}][total_no_of_bags]" id="total_no_of_bags_${i}"`;
            html += `                   class="form-control" required />`;
            html += `        </div>`;
            html += `        <div class="col-4">`;
            html += `            <label>Kgs Per Bag / Box(Bag / Box Weight)<b class="text-danger">*</b></label>`;
            ...
            html += `                    <th>CONE TIP COLOR</th>`;
            html += `                    <th>LOT NO.</th>`;
            html += `                    <th>NO. OF BAGS / BOX<b class="text-danger">*</b></th>`;
            html += `                    <th>KGS PER BAG / BOX<b class="text-danger">*</b></th>`;
            html += `                    <th>TOTAL KGS<b class="text-danger">*</b></th>`;
            html += `                    <th>CONES PER BAG / BOX<b class="text-danger">*</b></th>`;
            html += `                    <th>`;
