    ...
    <div class="card card-custom" style="box-shadow: none">
        <div class="card-header flex-wrap border-0 pt-0 pb-0">
            <div class="card-title">
                <h3 class="card-label">
                    YARN PURCHASE ORDER APPROVAL LIST
                </h3>
            </div>
        </div>
        <hr>
        <div class="ml-10">
            <a class="btn @if(empty($type) && $type !== '0') btn-linkedin @else btn-outline-info @endif"
               href="{{ route('approval_yarn_po') }}" role="button">Pending</a>
            <a class="btn @if(!empty($type) && $type == 1) btn-linkedin @else btn-outline-info @endif"
               href="{{ route('approval_yarn_po',1) }}" role="button">Approved</a>
            <a class="btn @if(empty($type) && $type == '0') btn-linkedin @else btn-outline-info @endif"
               href="{{ route('approval_yarn_po',0) }}" role="button">On Hold</a>
            <a class="btn @if(!empty($type) && $type == -1) btn-linkedin @else btn-outline-info @endif"
               href="{{ route('approval_yarn_po',-1) }}" role="button">Rejected</a>
        </div>
                ...
                @if (!empty($list))
                    @foreach ($list as $key => $item)
                        ...
                        <tr>
                            ...
                            <td>{{ $final_price }}</td>
                            <td class="d-flex">
                                <a href="{{ route('approval_yarn_po_get', $item->id) }}"
                ...
                @if (!empty($list2))
                    @foreach ($list2 as $key => $item)
                        @php
                            ...
                            $final_price = 0.00;
                            foreach ($item->details as $row) {
                                $counts .= $row->yarn?->name.', ';
                                $quantity += $row->total_cones;
                                $final_price += $row->basic_amount;
                            }
                        @endphp
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            ...
                            <td>{{ $final_price }}</td>
                            <td class="d-flex">
                                <a href="{{ route('approval_yarn_inward_get', $item->id) }}"
                                   class="btn btn-outline-info btn-sm btn-clean  " title="Select">
                                    Select
                                </a>
                            </td>
