...
@section('content')
    @php
        $route_main = 'variety';
        $module_label = 'Variety';
        $permission_name = 'yarn_variety';
    @endphp
                    ...
                    @if (!empty($varieties))
                        @foreach ($varieties as $key => $variety)
                            <tr>
                                ...
                                <td>{{ $variety->code }}</td>
                                <td>
                                <span class="label label-lg font-weight-bold label-inline label-light-{{ $variety->status ?  'success': 'danger' }} ">
                                {{ $variety->status ?  'Active': 'Inactive' }}</span>
                                </td>
                                <td class="d-flex">
                                @can($permission_name.'-update')
                                    <a href="javascript:void(0)" data-id="{{ $variety->id }}"
                                       class="btn btn-warning btn-sm btn-clean btn-icon btn-edit" title="Edit details">
                                        <i class="la la-edit"></i>
                                    </a>
                                @endcan
                                @can($permission_name.'-delete')
                                    <a href="javascript:void(0)" data-id="{{ $variety->id }}" title="Delete"
                    ...
                    <div class="form-group row">
                        <div class="col-12">
                            ...
                            <input type="text" name="code" class="form-control" id="code" autocomplete="off"
                                placeholder="Enter Code" value="" required />
                        </div>
                    </div>
