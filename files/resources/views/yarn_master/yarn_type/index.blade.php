...
@section('content')
    @php
        $route_main = 'yarn_type';
        $module_label = 'Yarn Type';
        $permission_name = 'yarn_type';
    @endphp
                    ...
                    @if (!empty($types))
                        @foreach ($types as $key => $type)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $type->type }}</td>
                                <td>{{ $type->units?->code ?? '' }}</td>
                                <td>{{ $type->factor }}</td>
                                <td>{{ $type->system }}</td>
                                <td>
                                <span class="label label-lg font-weight-bold label-inline label-light-{{ $type->status ?  'success': 'danger' }} ">
                                {{ $type->status ?  'Active': 'Inactive' }}</span>
                                </td>
                                <td class="d-flex">
                                ...
                                @can($permission_name.'-delete')
                                    <a href="javascript:void(0)" data-id="{{ $type->id }}" title="Delete"
...
{{--        {{ html()->form('POST', '/yarn_type')->acceptsFiles()->open() }}--}}
        {{ html()->form('POST', '/'.$route_main)->acceptsFiles()->id('storeForm')->open() }}
                    ...
                    <div class="form-group row">
                        <div class="col-12">
                            ...
                            <input type="text" name="type" class="form-control" id="type" autocomplete="off"
                                placeholder="Enter Type" value="" required />
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        <div class="col-12">
                            ...
                            <input type="text" name="factor" class="form-control" id="factor"
                                autocomplete="off" placeholder="Enter Conversion Factor" value=""
                                required />
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        <div class="col-12">
                            ...
                            <input type="text" name="loss" class="form-control" id="loss"
                                autocomplete="off" placeholder="Enter Weight Loss%" value="" />
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        <div class="col-12">
                            ...
                            <input type="text" name="code" class="form-control" id="code"
                                autocomplete="off" placeholder="Enter Yarn Code" value=""
                                required />
                        </div>
                    </div>
