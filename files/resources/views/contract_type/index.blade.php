...
@section('content')
    @php
        $route_main = 'contract_type';
        $module_label = 'Contract type';
        $permission_name = 'contract_type';
    @endphp
...
{{--                            <div class="form-group row">--}}
                                <table class="table">
                                    <tr class="table-primary">
                                        <th>Contract OrderRoute Name</th>
                                        <th>Contract OrderRoute Code</th>
                                        <th>Show in PPC</th>
                                        <th>Show in Fabric PO</th>
                                        <th>Show In Warehouse</th>
                                        <th>Bank Purpose</th>
                                        <th>
                                            <button
...
@push('scripts')
    ...
    <script>
        ...
        const add_order_details = (data = false) => {
            let index = $('#details_container tr').length;
            var html = ``;
            html += ``;
            html += `<tr>`;
            html += `    <td>`;
            html += `        <input type="text" name="details[${index}][contract_order_route_name]"`;
            html += `               class="form-control" value="${data.contract_order_route_name ?? ''}"/>`;
            html += `    </td>`;
            html += `    <td>`;
            html += `        <input type="text" name="details[${index}][contract_order_route_code]"`;
            html += `               class="form-control" value="${data.contract_order_route_code ?? '' }"/>`;
            html += `    </td>`;
            html += `    <td>`;
            html += `        <input type="checkbox" name="details[${index}][show_in_ppc]"`;
            html += `               class="form-control w-20px" value="1" ${ data.show_in_ppc == '1' ? 'checked' : '' }/>`;
            html += `    </td>`;
            html += `    <td>`;
            html += `        <input type="checkbox" name="details[${index}][show_in_fabric_po]"`;
            html += `               class="form-control w-20px" value="1" ${ data.show_in_fabric_po == '1' ? 'checked' : '' }/>`;
            html += `    </td>`;
            html += `    <td>`;
            html += `        <input type="checkbox" name="details[${index}][show_in_warehouse]"`;
            html += `               class="form-control w-20px" value="1" ${ data.show_in_warehouse == '1' ? 'checked' : '' }/>`;
            html += `    </td>`;
            html += `    <td>`;
            html += `        <input type="checkbox" name="details[${index}][bank_purpose]"`;
            html += `               class="form-control w-20px" value="1" ${ data.bank_purpose == '1' ? 'checked' : '' }/>`;
            html += `    </td>`;
            html += `    <td>`;
            html += `        <button class="btn btn-icon btn-danger btn-xs btn-circle d-block"`;
