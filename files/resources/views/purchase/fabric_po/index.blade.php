...
@section('content')
    @php
        $route_main = 'fabric_po';
        $module_label = 'Fabric-PO';
        $permission_name = 'fabric_po';
    @endphp
    <div class="card card-custom" style="box-shadow: none">
        ...
        <div class="ml-10">
            <a class="btn @if(empty($type) && $type !== '0') btn-linkedin @else btn-outline-info @endif"
               href="{{ route('fabric_po_list') }}" role="button">Pending</a>
            <a class="btn @if(!empty($type) && $type == 1) btn-linkedin @else btn-outline-info @endif"
               href="{{ route('fabric_po_list',1) }}" role="button">Approved</a>
            <a class="btn @if(empty($type) && $type == '0') btn-linkedin @else btn-outline-info @endif"
               href="{{ route('fabric_po_list',0) }}" role="button">On Hold</a>
            <a class="btn @if(!empty($type) && $type == -1) btn-linkedin @else btn-outline-info @endif"
               href="{{ route('fabric_po_list',-1) }}" role="button">Rejected</a>
        </div>
