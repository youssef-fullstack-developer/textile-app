        ...
        <div class="card-body">
            <table class="table table-bordered table-hover" id="table_invoice_settings">
                ...
                <tr class="text-uppercase">
                    <th>id</th>
                    <th class="pl-7"><span class="text-dark-75">Agent Name</span></th>
                    <th class="pl-7"><span class="text-dark-75">Country</span></th>
                    <th class="pl-7"><span class="text-dark-75">Primary Contact No</span></th>
                    <th class="pl-7"><span class="text-dark-75">Secondary Contact No</span></th>
                ...
                @if (!empty($list))
                    @foreach ($list as $key => $item)
                        <tr>
                            ...
                            <td>{{ $item->primary_contact_no }}</td>
                            <td>{{ $item->secondary_contact_no }}</td>
                            <td>{{ $item->address }}</td>
                            <td>{{ $item->pin_code }}</td>
                            <td>{{ $item->agent_percent }}</td>
                            <td>
                                <span
...
{{--                    <div class="row">--}}
                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                <input type="text" name="primary_contact_no" id="primary_contact_no" class="form-control"/>
                            </div>
                            <div class="col-6">
                                <label>Secondary Contact No</label>
                                <input type="text" name="secondary_contact_no" id="secondary_contact_no" class="form-control"/>
                            </div>
                        </div>
                        {{-- Line 3 --}}
                        <div class="row">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6">
                                <label>Agent Type<b class="text-danger">*</b></label>
                                <select name="agent_type_id" id="agent_type_id" class="form-control" required>
                                    <option value="">Select</option>
                                    @foreach($agent_types as $row)
                                        <option value="{{$row->id}}">{{$row->name}}</option>
                                    @endforeach
                                </select>
                            </div>
        ...
        $(document).ready(function () {
            ...
            $('.btn-edit').on('click', function () {
                ...
                $.get('/{{$route_main}}/' + id + '/edit', function (data) {
                    ...
                    $('#primary_contact_no').val(data.primary_contact_no);
                    $('#secondary_contact_no').val(data.secondary_contact_no);
                    $('#address').val(data.address);
                    $('#pin_code').val(data.pin_code);
                    $('#agent_percent').val(data.agent_percent);
                    $('#agent_type_id').val(data.agent_type_id);
