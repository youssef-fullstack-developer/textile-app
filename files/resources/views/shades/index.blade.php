                    ...
                    @if (!empty($list))
                        @foreach ($list as $key => $row)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $row->name }}</td>
                                <td>{{ $row->parent_sort?->sort_no }}</td>
                                <td>{{ $row->actual_sort?->sort_no }}</td>
                                <td class="d-flex">
                                    ...
                                    <input type="hidden" name="id" value="{{$row->id}}">
                                        ...
                                        >Create</button>
{{--                                    <a href="{{ route('shades.duplicate', $row->id) }}"--}}
{{--                                        class="btn btn-outline-light btn-sm btn-clean" title="Edit details">--}}
{{--                                        Create--}}
{{--                                    </a>--}}
                                    {{ html()->form()->close() }}
                                    @endif
