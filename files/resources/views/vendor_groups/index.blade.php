...
@section('content')
    @php
        $route_main = 'vendor_groups';
        $module_label = 'Vendor Group';
        $permission_name = 'vendor_group';
    @endphp
        ...
        <div class="card-body">
            <table class="table table-bordered table-hover" id="table_invoice_settings">
                ...
                <tr class="text-uppercase">
                    ...
                    <th class="pl-7"><span class="text-dark-75">Type</span></th>
                    <th class="pl-7"><span class="text-dark-75">Code</span></th>
                    <th class="pl-7"><span class="text-dark-75">Is Internal Group</span></th>
                ...
                @if (!empty($list))
                    @foreach ($list as $key => $item)
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>{{ $item->group }}</td>
                            <td>{{ $item->get_group_type?->name ?? '' }}</td>
                            <td>{{ $item->code }}</td>
                            <td>
                                <span
                                    class="label label-lg font-weight-bold label-inline label-light-{{ $item->is_internal ?  'success': 'danger' }} ">
                                {{ $item->is_internal ?  'Yes': 'No' }}</span>
                            </td>
                            <td>
                                <span
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <div class="col-12">
                            <label>Name</label>
                            <input type="text" name="group" id="group" class="form-control" placeholder="Enter Name"
                                   required/>
                        </div>
                    </div>
                    <div class="form-group row">
                    ...
                    <div class="form-group row">
                        <div class="col-12">
                            <label>Is Internal Group</label>
                            <div class="col-form-label">
                                <div class="radio-inline">
                                    <label class="radio radio-success">
                                        <input type="radio" name="is_internal" id="is_internal_true" checked="checked"
                                               value="1"/>
                                        <span></span>
                                        Yes
                                    </label>
                                    <label class="radio radio-danger">
                                        <input type="radio" name="is_internal" id="is_internal_false" value="0"/>
                                        <span></span>
                                        No
                                    </label>
                                </div>
    ...
    <script>
        $(document).ready(function () {
            $('#storeCurrentModel').on('hidden.bs.modal', function () {
                ...
                $('#group_type').val('');
                $('#code').val('');
                $('#is_internal_true').prop("checked", true);
                $('#is_internal_false').prop("checked", false);
            ...
            $('.btn-edit').on('click', function () {
                ...
                $.get('/{{$route_main}}/' + id + '/edit', function (data) {
                    ...
                    $('#group_type').val(data.group_type);
                    $('#code').val(data.code);
                    if (data.is_internal === true || data.is_internal === 1 || data.is_internal == '1') {
                        $('#is_internal_true').prop("checked", true);
                        $('#is_internal_false').prop("checked", false);
                    } else {
                        $('#is_internal_true').prop("checked", false);
                        $('#is_internal_false').prop("checked", true);
                    }
